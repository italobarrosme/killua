...
export const ButtonAuth = ({ icon = '', data, onClick,className, ...props}: ButtonAuthProps) => {
  return (
    <button onClick={onClick} className={clsx('appearance-none cursor-pointer', data ? 'flex items-center gap-4 border rounded-3xl p-2' : 'font-bold text-brand-secondary p-2' )} {...props}>
      {data ? (<>
